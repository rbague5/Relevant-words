...
def train_gmm_model(w2v_model, nouns, model_path):
    ...
    for n_clusters in range(10, 60, 10):
        ...
        closest[model_name] = get_top_10_nearest_points(gmm, w2v_model, rich_corpus)
...
def get_top_10_nearest_points(gmm_model, w2v_model, rich_corpus):
    top_n = 10
    w, h  = top_n, len(gmm_model.means_)
    top_10 = [[0 for x in range(w)] for y in range(h)]
    for n in range(w):
        embedding_corpus = np.array([w2v_model.wv[key] for key in rich_corpus])
        closest_idx, _ = pairwise_distances_argmin_min(gmm_model.means_, embedding_corpus)
        closest_words = [rich_corpus[idx] for idx in closest_idx.tolist()]
        for idx, val in enumerate(closest_words):
            top_10[idx][n] = val
        rich_corpus = list(filter(lambda x: x not in closest_words, rich_corpus))
    return top_10
